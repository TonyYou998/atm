...
int pcs_import_private_key(pcs_private_key *vk, const char *json)
{
    ...
    /* Calculate remaining values */
    mpz_pow_ui(vk->p2, vk->p, 2); // p2 = p **2
    mpz_pow_ui(vk->q2, vk->q, 2); // q2 = q **2
    mpz_mul(vk->n, vk->p, vk->q); // n = p * q
    mpz_sub_ui(vk->lambda, vk->p, 1); // lambda = p - 1
    mpz_sub_ui(vk->q, vk->q, 1); // q = q -1
    mpz_lcm(vk->lambda, vk->lambda, vk->q); // lambda = lcm(lambda, q)
    mpz_add_ui(vk->q, vk->q, 1);// q = q + 1
    mpz_pow_ui(vk->n2, vk->n, 2); // n2 = n ** 2
    mpz_invert(vk->mu, vk->lambda, vk->n); // mu * lambda dong du 1 mod n

    mpz_add_ui(g, vk->n, 1); // g = n + 1

    mpz_sub_ui(vk->hp, vk->p, 1); // hp = p - 1
    mpz_powm(vk->hp, g, vk->hp, vk->p2); // hp  = (g ** hp) mod p2
    mpz_sub_ui(vk->hp, vk->hp, 1); // hp  = hp - 1
    mpz_tdiv_q(vk->hp, vk->hp, vk->p); // hp = trunc(hp / p)
    mpz_invert(vk->hp, vk->hp, vk->p); // hp * hp dong du 1 mod p
    
    mpz_sub_ui(vk->hq, vk->q, 1);
    mpz_powm(vk->hq, g, vk->hq, vk->q2); // hp  = (g ** hq) mod p2
    mpz_sub_ui(vk->hq, vk->hq, 1);
    mpz_tdiv_q(vk->hq, vk->hq, vk->q);
    mpz_invert(vk->hq, vk->hq, vk->q);
